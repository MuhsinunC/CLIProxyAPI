			...
			if (role == "system" || role == "developer") && len(arr) > 1 {
						...
						case "tool_result":
							...
							if toolUseID != "" {
								...
								if resultContent.Type == gjson.String {
									node, _ = sjson.SetRawBytes(node, "parts."+itoa(p)+".functionResponse.response", []byte(`{"result":`+resultContent.Raw+`}`))
								} else if resultContent.IsArray() || resultContent.IsObject() {
									node, _ = sjson.SetRawBytes(node, "parts."+itoa(p)+".functionResponse.response", []byte(resultContent.Raw))
								}
								node, _ = sjson.SetBytes(node, "parts."+itoa(p)+".thoughtSignature", geminiCLIFunctionThoughtSignature)
								p++
							}
				...
				if content.Type == gjson.String && content.String() != "" {
					...
					for _, item := range content.Array() {
						...
						case "tool_use":
							// Claude-format tool_use in assistant content array -> functionCall
