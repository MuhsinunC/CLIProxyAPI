...
func ConvertOpenAIRequestToClaude(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	// Response format handling: Convert json_schema/json_object to tool injection
	// Only applies when response_format.type is "json_schema" or "json_object" AND no other tools are present
	if responseFormat := root.Get("response_format"); responseFormat.Exists() {
		...
		if !hasExistingTools {
			...
			if rfType == "json_schema" {
				...
				// Create a generic schema that accepts any JSON object
			...
			// If we have a valid tool configuration, inject it
