...
def send_request(messages: list, tools: Optional[list] = None, stream: bool = False) -> dict:
    ...
    if stream:
        response = requests.post(PROXY_URL, headers=headers, json=payload, stream=True, timeout=120)
...
def extract_thinking_and_tool_calls(response: dict):
    ...
    if "choices" in response and response["choices"]:
        ...
        result["thinking"] = message.get("reasoning_content") or message.get("reasoning")
...
def test_scenario_1_only_last_thinking_block():
    ...
    messages = [
        {"role": "system", "content": "You are a helpful assistant. Always use the get_current_time tool when asked about time."},
    ...
    if len(messages) > 4:
        messages_without_old_thinking.append({  # assistant turn 2 - WITHOUT reasoning_content
            "role": "assistant",
            "content": turn2_data['content'] or "The current time is 10:30 AM.",
        })
    ...
    log(f"Sending request with {len(messages_without_old_thinking)} messages (old thinking removed)...")
...
def test_scenario_2_no_thinking_blocks():
    ...
    try:
        ...
        if "error" in response:
            ...
            log("RESULT: SUCCESS - Request completed without error")
            log("CONCLUSION: Claude does NOT require thinking blocks (or thinking is disabled)")
            return True
    except Exception as e:
        log(f"RESULT: EXCEPTION - {e}")
        return False
...
def main():
    parser = argparse.ArgumentParser(description="Test Claude thinking block requirements")
    ...
    if results.get("scenario_1_only_last"):
        ...
        log("- Cache strategy: Store only the most recent thinking block per conversation")
